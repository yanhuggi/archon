...
另有一处跨进程延迟：一个 MCP 进程因 JQL 校验失败而作废快照后，其他进程最多约 30 秒才会观察到。失效标记通过 `stat` 快路径检查以避免每次缓存命中都读盘，指纹未变时最长 30 秒强制重新解析一次，作为 inode 复用或时间戳粒度粗的挂载上的兜底。这是性能与一致性的取舍；需要立刻生效时使用 `refresh=true`。
...
输出文件的发布策略按平台能力降级，任何一档都不会暴露半个文件：

| 条件 | 行为 |
|---|---|
| 支持硬链接 | `os.link` 一次完成「创建且不覆盖」 |
| 不支持硬链接，有 `renameat2` | 写入唯一临时文件后 `RENAME_NOREPLACE` 原子移动 |
| 两者都不可用 | 检查目标是否存在后 `os.rename`；检查与改名之间存在极小窗口，另一进程恰好在此期间创建目标时可能被替换。这是缺少该系统调用时无法消除的窗口 |

临时文件名每次唯一，进程崩溃留下的残留不会阻塞后续导出。进程被强制终止时残留的
暂存文件只占用磁盘，每次导出会顺带清理同目录中超过 6 小时未修改的残留。清理只匹配
本服务自己创建的 `.archon-jira-publish-*.publish`，不会触及输出目录中用户的同后缀
文件；清理是尽力而为，失败不影响本次导出，也不会误删正在写入的暂存文件。
